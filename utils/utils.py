...
def collate_MIL_survival(batch):
    # print(len(batch))
    # print(len(batch[0]))
    # print([item[3] for item in batch])
    # print([item[4] for item in batch])
    img = torch.cat([item[0] for item in batch], dim = 0)
    rad_A = torch.cat([item[1] for item in batch], dim = 0).type(torch.FloatTensor)
    rad_V = torch.cat([item[2] for item in batch], dim = 0).type(torch.FloatTensor)
    rad_A_seg = torch.cat([item[3] for item in batch], dim = 0).type(torch.FloatTensor)
    rad_V_seg = torch.cat([item[4] for item in batch], dim = 0).type(torch.FloatTensor)
    label = torch.FloatTensor([item[5] for item in batch])
    event_time = torch.FloatTensor([item[6] for item in batch])
    c = torch.FloatTensor([item[7] for item in batch])
    #rad_all = torch.cat([item[8] for item in batch], dim = 0).type(torch.FloatTensor)
    # return [img, rad_A, rad_V,rad_A_seg,rad_V_seg,label, event_time, c, rad_all]
    return [img, rad_A, rad_V,rad_A_seg,rad_V_seg,label, event_time, c]
