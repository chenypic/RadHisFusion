...
from models.model_RadHisFusion import RadHisFusion
...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    test_survival(cur, epoch, model, test_loader, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir)
...
def train_loop_survival(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16):   
    ...
    for batch_idx, (data_WSI, rad_A,rad_V, rad_A_seg,rad_V_seg,y_disc, event_time, censor) in enumerate(loader):  
        ...
        h,x1_out,x2_out = model(x_path=data_WSI, rad_A = rad_A, rad_V=rad_V)
...
def validate_survival(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    ...
    for batch_idx, (data_WSI, rad_A,rad_V, rad_A_seg,rad_V_seg, y_disc, event_time, censor) in enumerate(loader):
        ...
        with torch.no_grad():
            h,x1_out,x2_out = model(x_path=data_WSI, rad_A = rad_A, rad_V=rad_V)# return hazards, S, Y_hat, A_raw, results_dict
...
def test_survival(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    val_loss_surv, val_loss = 0., 0.
    all_risk_scores = np.zeros((len(loader)))
