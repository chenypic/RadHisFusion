...
from models.model_porpoise_rad import PorpoiseMMF, PorpoiseAMIL
from utils.utils import *
from utils.loss_func import NLLSurvLoss,EDiceLoss
...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    train_split, val_split,val_split_internal,test_split,test_split_2 = datasets
    #      save_splits(datasets, ['train', 'val'], os.path.join(args.results_dir, 'splits_{}.csv'.format(cur)))
    print('Done!')
    print("Training on {} samples".format(len(train_split)))
    print("Validating on {} samples".format(len(val_split)))
    print("Validating on internal {} samples".format(len(val_split_internal)))
    print("Validating on liaocheng {} samples".format(len(test_split)))
    print("Validating on yantai {} samples".format(len(test_split_2)))
    ...
    val_loader_internal = get_split_loader(val_split_internal,  testing = args.testing, mode=args.mode, batch_size=args.batch_size)
    test_loader = get_split_loader(test_split,  testing = args.testing, mode=args.mode, batch_size=args.batch_size)
    test_loader_yantai = get_split_loader(test_split_2,  testing = args.testing, mode=args.mode, batch_size=args.batch_size)
    print('Done!')
    ...
    for epoch in range(args.max_epochs):
        if args.task_type == 'survival':
            if args.mode == 'coattn':
                ...
                validate_survival_internal(cur, epoch, model, val_loader_internal, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir)
                validate_survival_2(cur, epoch, model, test_loader, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir)
                validate_survival_yantai(cur, epoch, model, test_loader_yantai, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir)

        torch.save(model.state_dict(), os.path.join(args.results_dir, "s_{:0>3d}_checkpoint.pt".format(epoch)))
...
def train_loop_survival(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., gc=16):   
    ...
    for batch_idx, (data_WSI, rad_A,rad_V, rad_A_seg,rad_V_seg,y_disc, event_time, censor) in enumerate(loader):  

        
        # data_WSI, data_omic = data_WSI.to(device), data_omic.to(device) 
        # 屏蔽掉原先的基因部分代码，换成影像部分的
        data_WSI = data_WSI.to(device)

        rad_A = rad_A.to(device)
        rad_V = rad_V.to(device)

        y_disc = y_disc.to(device)
        event_time = event_time.to(device)
        censor = censor.to(device)

        rad_A_seg,rad_V_seg = rad_A_seg.to(device),rad_V_seg.to(device)
        ...
        h,x1_out,x2_out = model(rad_A = rad_A, rad_V=rad_V)

        loss_ = criterion(x1_out, rad_A_seg) + criterion(x2_out, rad_V_seg)
        ...
        # backward pass
        loss = loss / gc + loss_reg+loss_ /gc
        loss.backward()
...
def validate_survival(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    ...
    for batch_idx, (data_WSI, rad_A,rad_V, rad_A_seg,rad_V_seg, y_disc, event_time, censor) in enumerate(loader):
        ...
        with torch.no_grad():
            h,x1_out,x2_out  = model(rad_A = rad_A, rad_V=rad_V)# return hazards, S, Y_hat, A_raw, results_dict
    ...
    p_value = cox_log_rank(all_risk_scores, (1-all_censorships).astype(bool), all_event_times)

    print('训练集--省立旧+泰安的结果为：')
    print('c-index: ', c_index)
    print('p-value: ',p_value)

    data = {'risk':all_risk_scores,
       'status':1-all_censorships,
       'time':all_event_times}
    df = pd.DataFrame(data)
    df.to_csv(os.path.join(results_dir, "s_{:0>3d}_train_{:.4f}_{:.4f}.csv".format(epoch,c_index,p_value)))
...
def validate_survival_2(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    val_loss_surv, val_loss = 0., 0.
    all_risk_scores = np.zeros((len(loader)))
    ...
    data = {'risk':all_risk_scores,
       ...
       'time':all_event_times}
    df = pd.DataFrame(data)
    df.to_csv(os.path.join(results_dir, "s_{:0>3d}_liaocheng_{:.4f}_{:.4f}.csv".format(epoch,c_index,p_value)))


    if writer:
        writer.add_scalar('test/loss_surv', val_loss_surv, epoch)
        writer.add_scalar('test/loss', val_loss, epoch)
        writer.add_scalar('test/c-index', c_index, epoch)


    return False


def validate_survival_yantai(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    val_loss_surv, val_loss = 0., 0.
    all_risk_scores = np.zeros((len(loader)))
    ...
    data = {'risk':all_risk_scores,
       ...
       'time':all_event_times}
    df = pd.DataFrame(data)
    df.to_csv(os.path.join(results_dir, "s_{:0>3d}_yantai_{:.4f}_{:.4f}.csv".format(epoch,c_index,p_value)))



    if writer:
        writer.add_scalar('test_yantai/loss_surv', val_loss_surv, epoch)
        writer.add_scalar('test_yantai/loss', val_loss, epoch)
        writer.add_scalar('test_yantai/c-index', c_index, epoch)
...
def validate_survival_internal(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None, loss_fn=None, reg_fn=None, lambda_reg=0., results_dir=None):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    val_loss_surv, val_loss = 0., 0.
    all_risk_scores = np.zeros((len(loader)))
    ...
    data = {'risk':all_risk_scores,
       ...
       'time':all_event_times}
    df = pd.DataFrame(data)
    df.to_csv(os.path.join(results_dir, "s_{:0>3d}_val_inter_{:.4f}_{:.4f}.csv".format(epoch,c_index,p_value)))



    if writer:
        writer.add_scalar('val_inter/loss_surv', val_loss_surv, epoch)
        writer.add_scalar('val_inter/loss', val_loss, epoch)
        writer.add_scalar('val_inter/c-index', c_index, epoch)
...
def summary_survival(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        censor = censor.detach().cpu().numpy()
        all_risk_scores[batch_idx] = risk
