...
class PorpoiseMMF(nn.Module):
    def __init__(self, 
        ...
        self.backbone1 = resnet.resnet18(in_channels=1, sample_input_W=1, sample_input_H=1, sample_input_D=1, shortcut_type='A',
            num_seg_classes=2)
        self.backbone2 = resnet.resnet18(in_channels=1, sample_input_W=1, sample_input_H=1, sample_input_D=1, shortcut_type='A',
            num_seg_classes=2)

        self.upconv1 = nn.Sequential(
            nn.ConvTranspose3d(512, 64, kernel_size=2, stride=2),
            nn.PReLU()
        ).cuda()
        ...
        self.conv1_out = nn.Conv3d(
            in_channels=64, out_channels=1, kernel_size=1
        ).cuda()
        self.conv2_out = nn.Conv3d(
            in_channels=64, out_channels=1, kernel_size=1
        ).cuda()
        ...
        self.FDR1 = nn.Linear(512, 256).cuda()
        self.FDR2 = nn.Linear(512, 256).cuda()

        self.transformer_encoder_layer = TransformerEncoderLayer(d_model=256, nhead=8).cuda()
        self.transformer_encoder = TransformerEncoder(self.transformer_encoder_layer, num_layers=2).cuda()

        self.mp = nn.MaxPool1d(2, stride=1).cuda()
    ...
    def forward(self, **kwargs):
        ...
        x1_out = self.upconv1(x1)
        x2_out = self.upconv2(x2)

        x1_out = self.conv1_out(x1_out)
        x2_out = self.conv2_out(x2_out)

        # print('x1_out',x1_out.size())
        # print('x2_out',x1_out.size())
        ...
        x1 = self.FDR1(x1) 
        x2 = self.FDR2(x2) 
        x = torch.cat((x1.view(x1.size(0), 1, -1), x2.view(x1.size(0), 1, -1)), dim=1)
        x = self.transformer_encoder(x)
        h_omic = self.mp(x.permute(0, 2, 1)).view(x.size(0), -1)

        h_mm  = self.classifier_mm(h_omic) # logits needs to be a [B x 4] vector      
        assert len(h_mm.shape) == 2 and h_mm.shape[1] == self.n_classes


        return h_mm,x1_out,x2_out
