...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        slide_data = pd.read_csv(csv_path, low_memory=False,index_col=0)
        ...
        metadata = ['disc_label', 'svsFile', 'slide_id', 'label', 'censorship', 'survival_months', 'ids_1', 'ids_2', 'case_id']

        metadata = ['disc_label', 'svsFile', 'slide_id', 'label', 'censorship', 'survival_months', 'case_id','radiology']
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    genomic_features = self.genomic_features.iloc[idx] # genomic_features 改为影像文件名的列表

                   

                    rad_A = genomic_features[0]
                    rad_V = genomic_features[1]

                    rad_A_seg = genomic_features[0].replace('image','seg')
                    rad_V_seg = genomic_features[1].replace('image','seg')
                    ...
                    rad_A = sitk.ReadImage(rad_A)
                    rad_V = sitk.ReadImage(rad_V)
                    rad_V = intensityWindow.Execute(rad_V)
                    rad_A = intensityWindow.Execute(rad_A)

                    rad_A = sitk.GetArrayFromImage(rad_A).astype(np.float32)
                    rad_V = sitk.GetArrayFromImage(rad_V).astype(np.float32)

                    rad_A_seg = sitk.ReadImage(rad_A_seg)
                    rad_V_seg = sitk.ReadImage(rad_V_seg)
                    rad_A_seg = sitk.GetArrayFromImage(rad_A_seg).astype(np.float32)
                    rad_V_seg = sitk.GetArrayFromImage(rad_V_seg).astype(np.float32)

                    # print('rad_A_seg',rad_A_seg.shape)
                    # print('rad_V_seg',rad_V_seg.shape)

                    # if rad_V_seg.shape[0] ==31:
                    #     rad_V_seg = np.vstack((rad_V_seg,np.zeros_like(rad_V_seg[0])[ np.newaxis,]))
                    #     rad_V = np.vstack((rad_V,rad_V[-1][ np.newaxis,]))

                    # if rad_A_seg.shape[0] ==31:
                    #     rad_A_seg = np.vstack((rad_A_seg,np.zeros_like(rad_A_seg[0])[ np.newaxis,]))
                    #     rad_A = np.vstack((rad_A,rad_A[-1][ np.newaxis,]))


                    if rad_V_seg.shape[0] !=32:
                        cha_de = 32-rad_V_seg.shape[0]
                        for j in range(cha_de):
                            rad_V_seg = np.vstack((rad_V_seg,np.zeros_like(rad_V_seg[0])[ np.newaxis,]))
                            rad_V = np.vstack((rad_V,rad_V[-1][ np.newaxis,]))

                    if rad_A_seg.shape[0] !=32:
                        cha_de = 32-rad_A_seg.shape[0]
                        for j in range(cha_de):
                            rad_A_seg = np.vstack((rad_A_seg,np.zeros_like(rad_A_seg[0])[ np.newaxis,]))
                            rad_A = np.vstack((rad_A,rad_A[-1][ np.newaxis,]))

                    if rad_A.shape[2] !=128:
                        cha_de = 128-rad_A.shape[2]
                        for j in range(cha_de):
                            rad_A = np.concatenate((rad_A,rad_A[:,:,-1][ ...,np.newaxis]),2)
                            rad_A_seg = np.concatenate((rad_A_seg,np.zeros_like(rad_A_seg[:,:,-1])[ ...,np.newaxis]),2)

                    if rad_A.shape[1] !=128:
                        cha_de = 128-rad_A.shape[1]
                        for j in range(cha_de):
                            rad_A = np.concatenate((rad_A,rad_A[:,-1,:][:,np.newaxis,:]),1)
                            rad_A_seg = np.concatenate((rad_A_seg,np.zeros_like(rad_A_seg[:,-1,:])[ :,np.newaxis,:]),1)
                    
                    if rad_V.shape[2] !=128:
                        cha_de = 128-rad_V.shape[2]
                        for j in range(cha_de):
                            rad_V = np.concatenate((rad_V,rad_V[:,:,-1][ ...,np.newaxis]),2)
                            rad_V_seg = np.concatenate((rad_V_seg,np.zeros_like(rad_V_seg[:,:,-1])[ ...,np.newaxis]),2)

                    if rad_V.shape[1] !=128:
                        cha_de = 128-rad_V.shape[1]
                        for j in range(cha_de):
                            rad_V = np.concatenate((rad_V,rad_V[:,-1,:][:,np.newaxis,:]),1)
                            rad_V_seg = np.concatenate((rad_V_seg,np.zeros_like(rad_V_seg[:,-1,:])[ :,np.newaxis,:]),1)



                    
                    rad_A_seg = zoom(rad_A_seg, (0.25, 0.25, 0.25), order=0)
                    rad_V_seg = zoom(rad_V_seg, (0.25, 0.25, 0.25), order=0)


                    # print('rad_A_seg',rad_A_seg.shape)
                    # print('rad_V_seg',rad_V_seg.shape)

                    # print(np.max(rad_A_seg))
                    # print(np.max(rad_V_seg))



                    rad_A = rad_A[..., np.newaxis]
                    rad_V = rad_V[..., np.newaxis]

                    rad_A = rad_A[..., np.newaxis]
                    rad_V = rad_V[..., np.newaxis]

                    rad_A = torch.from_numpy(rad_A).permute(4,3, 0, 1, 2)
                    rad_V = torch.from_numpy(rad_V).permute(4,3, 0, 1, 2)

                    rad_all = torch.concatenate((rad_A,rad_V),1)


                    rad_A_seg = rad_A_seg[..., np.newaxis]
                    rad_V_seg = rad_V_seg[..., np.newaxis]

                    rad_A_seg = rad_A_seg[..., np.newaxis]
                    rad_V_seg = rad_V_seg[..., np.newaxis]

                    rad_A_seg = torch.from_numpy(rad_A_seg).permute(4,3, 0, 1, 2)
                    rad_V_seg = torch.from_numpy(rad_V_seg).permute(4,3, 0, 1, 2)

                    # print('label*******',label)
                    # print('event_time*******',event_time)
                    # print('c*******',c)

                    return (path_features, rad_A,rad_V,rad_A_seg,rad_V_seg, label, event_time, c,rad_all)
